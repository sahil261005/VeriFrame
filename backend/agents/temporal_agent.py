...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python temporal_agent.py <path_to_video>")
        sys.exit(1)

    video = sys.argv[1]
    print(f"Testing temporal agent on: {video}")

    meta = validate_video(video)
    print(f"Video: {meta['duration']}s, {meta['fps']} fps")

    temp_path = "temp_temporal_test.mp4"
    downscale_video(video, temp_path)
    frames = extract_frames(temp_path, interval=0.5)
    print(f"Got {len(frames)} frames\n")

    score, flagged, flow, face = run_temporal_analysis(frames)

    print(f"\nTemporal Score: {score}")
    print(f"Flagged timestamps: {flagged}")

    print(f"\nOptical flow results ({len(flow)} transitions):")
    for r in flow:
        flag = " *** ANOMALY" if r["is_anomalous"] else ""
        print(f"  t={r['timestamp']}s  magnitude={r['flow_magnitude']}{flag}")

    if len(face) > 0:
        print(f"\nFace consistency results ({len(face)} comparisons):")
        for r in face:
            flag = " *** INCONSISTENT" if r["is_inconsistent"] else ""
            print(f"  t={r['timestamp']}s  shift={r['landmark_shift']}px{flag}")
    else:
        print("\nNo face consistency data (no faces found or mediapipe missing)")
